...
**Cotizador**: `src/pages/Cotizador` is a live podcast-quote calculator (table of services with qty/unit price/subtotal, plus two fixed quantity rows for extra mics/invitados and cámaras) — not just a lead-capture form. Services/prices live in `COTIZADOR_SERVICES` / `COTIZADOR_QUANTITY_ITEMS` in `copy.js`, sourced from real quote history (always use the most recent/highest price per service when updating). Services flagged `perEpisode: true` are multiplied by the chosen episode count. Reuses `formatPrice` and the subtotal→IVA(19%)→total pattern from `src/components/BookingFlow/useBookingFlow.js` rather than duplicating it.
